...
const BuyerFavourites = () => {
  ...
  const handleRemoveFavourite = async (propertyId) => {
    const result = await Swal.fire({
      title:
        '<span style="color:white; font-weight:bold; font-size: 1.5rem;">Remove Favorite?</span>',
      ...
      customClass: {
        popup:
          "border border-white/10 !rounded-[2.5rem] shadow-2xl backdrop-blur-xl",
        confirmButton:
          "bg-red-500 hover:bg-red-600 text-white px-8 py-3 rounded-full font-bold shadow-lg shadow-red-500/20 transition-all transform hover:scale-105",
        cancelButton:
          "bg-white/5 hover:bg-white/10 text-white px-8 py-3 rounded-full font-bold transition-all border border-white/10 hover:border-white/20",
        actions: "gap-6 !mt-6",
    ...
    return (
      <div className="flex flex-col items-center justify-center h-96 bg-white/5 rounded-3xl border border-white/10 backdrop-blur-md text-center p-6">
        ...
        <p className="text-gray-400 mb-8 max-w-sm">
          Start adding properties to your favorites list to keep track of your
          dream homes.
        </p>
        <Link
          to="/all-property"
          className="btn bg-orange-500 hover:bg-orange-600 text-white border-none rounded-full px-8 shadow-lg shadow-orange-500/20"
        >
          Browse Properties
        </Link>
      </div>
...
// PropertyCard Component - Fetches full property details
const PropertyCard = ({ favourite, onRemove }) => {
  const { data: propertyData, isLoading } = usePropertyById(
    favourite.propertyId,
  );

  if (isLoading) {
    return (
      <div className="bg-[#0f0f0f] border border-white/5 rounded-3xl overflow-hidden shadow-2xl h-full animate-pulse">
  ...
  const property = propertyData || favourite;

  return (
    <div className="bg-[#0f0f0f] border border-white/5 rounded-3xl overflow-hidden shadow-2xl hover:shadow-orange-500/20 transition-all duration-500 hover:-translate-y-2 group flex flex-col h-full relative">
      {/* Image Container */}
      <div className="relative h-64 overflow-hidden">
        ...
        <button
          onClick={() => onRemove(favourite.propertyId)}
      ...
      <div className="p-6 pt-0 flex flex-col grow relative z-20">
        ...
        <div className="mt-auto">
          <Link
            to={`/all-property/${favourite.propertyId}`}
